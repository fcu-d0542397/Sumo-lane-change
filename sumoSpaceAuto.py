...
def moveNonFleetPosition(times):
    ...
    for i in range(12):
        no = "no"+str(i)
        if i == 0:
            temp1 = firstNoFleetPosition
            temp2 = traci.vehicle.getPosition(no)[1]
            traci.vehicle.moveToXY(no, "1to2", 1, temp1, temp2)
        else:
            temp1 = temp1 - spaceArray[i-1]
            traci.vehicle.moveToXY(no, "1to2", 1, temp1, temp2)
...
def calFitFleetSpace(fleetLast):
    ...
    for i in range(matrixLength):
        for j in range(matrixLength):
            if spaceMatrix[i, j] > fleetLast + 2 * safty:
    ...
    print("satisSpace: "+str(satisSpace))
    # print("\n")
...
def sortingSpace():
    ...
    # print("minSatirange: "+str(minSatirange))
    # print("minSatiSpace: "+str(minSatiSpace))
    ...
    print("789: "+str(minSatiSpace))
    print("123: "+str(minSatirange))
...
def distributeSpace():
    ...
    for i in range(len(groupArray)):
        ...
        if timeDec != 0:
            ...
            solved2 = calACCSpeedDiff(
                otherPosition[start + i] - 4, fleetPosition[groupArray[i]], fleetSpeed[groupArray[i]], frontSpeed, timeDec, 1.5)
            print("sdsdsds: "+str(solved2))
            if solved2[0][0] > 0 and solved2[0][0] <= 8:
                indexAnswer = 0
                # noAnswerFlag = 1
                if solved2[0][2] < 0:
                    solved3 = calACCSpeedDiff2(
                        otherPosition[start + i] - 4, fleetPosition[groupArray[i]], fleetSpeed[groupArray[i]], frontSpeed, timeDec, 1.5)
                    if ":" in str(solved3):
                        characters = "{:x newSpeed eSafe}"
                        tempString = str(solved3)
                    for j in range(len(characters)):
                        tempString = tempString.replace(characters[j], "")
                    key = tempString
                ...
                noAnswerFlag = 1
                solved3 = calACCSpeedDiff3(
                    otherPosition[start + i] - 4, fleetPosition[groupArray[i]], fleetSpeed[groupArray[i]], frontSpeed, timeDec, 1.5)
                print("qeadzc: "+str(solved3))
                if ":" in str(solved3):
                    characters = "{: eSafe}"
                    tempString = str(solved3)
                for j in range(len(characters)):
                ...
                key = tempString
                if float(key) < 0:
                    tempSafe = fleetSpeed[groupArray[i]] * 1.5
                else:
                    tempSafe = float(key)
                solved4 = calACCSpeedDiff4(
                    otherPosition[start + i] - 4, fleetPosition[groupArray[i]], fleetSpeed[groupArray[i]], frontSpeed, timeDec, 1.5, tempSafe)
                print("ASDSAD: "+str(solved4))
                dec = 0
                groupSafty[i] = float(tempSafe)
                print("sdsdaqqqq: "+str(solved3))

            # else:
            #     print("dsadasdsadsadasdsad")
            #     print("7788 :"+str(solved2[indexAnswer]))
            #     dec = float(solved2[indexAnswer][0])
            #     groupSafty[i] = float(solved2[indexAnswer][2])
            ...
            if noAnswerFlag == 0:
                if solved2[indexAnswer][2] < 0:
                    solved2 = calACCSpeedDiff2(
                        otherPosition[start + i] - 4, fleetPosition[groupArray[i]], fleetSpeed[groupArray[i]], frontSpeed, timeDec, 1.5)
                    print("dddqqpr: "+str(solved2))
                    if ":" in str(solved2):
                        characters = "{:x newSpeed eSafe}"
...
def spaceFleetClose():
    ...
    for i in range(len(groupArray)):
        ...
        if i == len(groupArray) - 1:
            ...
            print(str(
                i)+" :"+str((otherPosition[start+i] - 4 - fleetPosition[groupArray[i]] - groupSafty[i])))
            # if fleetPosition[groupArray[i]] < otherPosition[start+i] and abs(otherPosition[start+i] - 4 - fleetPosition[groupArray[i]]-groupSafty[i]) < (frontSpeed2-nowSpeed2)*5+((frontSpeed2-nowSpeed2)/6)*(frontSpeed2-nowSpeed2) and groupChangeArray[i] == 0 and groupSafty[i] > 0:
            # if abs(otherPosition[start+i] - 4 - fleetPosition[groupArray[i]] - groupSafty[i]) < (frontSpeed2-nowSpeed2)*2 and groupChangeArray[i] == 0 and groupSafty[i] > 0:
            if abs(otherPosition[start+i] - 4 - fleetPosition[groupArray[i]] - groupSafty[i]) < 4 and groupChangeArray[i] == 0 and groupSafty[i] > 0:
                groupChangeTime[i] = step
...
def calACCSpeedDiff(locationX, groupPosition, nowSpeed, frontSpeed, t, notice):
    ...
    solve_value = solve([locationX - groupPosition - 4 - eSafe + (frontSpeed - newSpeed) * 1.5 - ((nowSpeed-frontSpeed)*t-(0.5*x*(t**2))),
                        newSpeed-(nowSpeed - x * t),
                        # eSafe - (((newSpeed**2 - frontSpeed**2) /(2 * 8)) + newSpeed*1.5)
                         # eSafe - newSpeed * notice
                         ((newSpeed**2 * 8) / (frontSpeed**2 + 2 * \
                          8*(eSafe - newSpeed * notice)) / 8) - 1
                         ], [x, newSpeed, eSafe],)
...
def calACCSpeedDiff3(locationX, groupPosition, nowSpeed, frontSpeed, t, notice):
    # solve_value = solve([(-10-46.0)-(nowSpeed-frontSpeed)*t-(0.5*x*(t**2))], [x])
    x = 0.001
    newSpeed = nowSpeed - x*t
    solve_value = solve([
                        # locationX - groupPosition - eSafe + (frontSpeed - newSpeed)* 1.5 -((nowSpeed-frontSpeed)*t-(0.5*x*(t**2))),
                        # newSpeed-(nowSpeed - x * t),
                        # eSafe - (((newSpeed**2 - frontSpeed**2) /(2 * 8)) + newSpeed*1.5)
                        # eSafe - newSpeed * notice
                        ((newSpeed**2 * 8) / (frontSpeed**2 + 2 * \
...
# 新計算減速度(安全距離)
def calACCSpeedDiff4(locationX, groupPosition, nowSpeed, frontSpeed, t, notice, eSafe):
    # solve_value = solve([(-10-46.0)-(nowSpeed-frontSpeed)*t-(0.5*x*(t**2))], [x])
